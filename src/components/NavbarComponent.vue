    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between">
        ...
        <router-link to="/" class="flex items-center gap-3 group">
          ...
          <span class="hidden sm:block font-bold text-lg transition-colors duration-200"
        ...
        <nav class="hidden lg:flex items-center gap-8">
          <a v-for="link in navLinks" :key="link.href" :href="link.href" :target="link.external ? '_blank' : undefined"
            :rel="link.external ? 'noopener noreferrer' : undefined"
            @click="link.external ? null : scrollToSection($event, link.href)"
            class="nav-link flex items-center gap-1.5" :class="[
